...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    advancedToggle.addEventListener("change", () => {
        if (advancedToggle.checked && typeof isAdvancedModeActive === "function" && !isAdvancedModeActive()) {
    ...
    async function loadConversations() {
        try {
            const response = await fetch(`${WORKER_BASE_URL}/ai/conversations`, {
    ...
    function renderConversationsList(conversations) {
        ...
        if (conversations.length === 0) {
            list.innerHTML = `<p class="ai-empty-note">Belum ada percakapan.</p>`;
            return;
        }
        ...
        conversations.forEach((conv) => {
            ...
            item.innerHTML = `
                ...
                <span class="ai-conv-delete" title="Hapus">🗑️</span>
            `;

            item.querySelector(".ai-conv-title").addEventListener("click", () => {
                openConversation(conv.id);
                sidebar.classList.remove("open");
            });

            item.querySelector(".ai-conv-delete").addEventListener("click", async (e) => {
                e.stopPropagation();
    ...
    async function deleteConversation(conversationId) {
        try {
            await fetch(`${WORKER_BASE_URL}/ai/conversations?conversationId=${conversationId}`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${getAccessToken()}` }
            });
            ...
            showMessageModal("Gagal", "Tidak dapat menghapus percakapan. Coba lagi.");
        }
    }
    ...
    async function openConversation(conversationId) {
        ...
        try {
            const response = await fetch(`${WORKER_BASE_URL}/ai/messages?conversationId=${conversationId}`, {
                headers: { Authorization: `Bearer ${getAccessToken()}` }
            });
            const data = await response.json();
    ...
    function resetChatView() {
        chatMessages.innerHTML = `
            <div class="ai-welcome-message">
                <h2>👋 Halo! Aku Smart Search AI</h2>
                <p>Ceritakan gambar seperti apa yang kamu cari, atau ajak aku ngobrol santai.</p>
            </div>
        `;
    }
    ...
    async function sendMessage() {
        ...
        try {
            ...
            if (!data.success) {
                renderMessage("assistant", data.message || "Maaf, terjadi kesalahan. Coba lagi.", null);
            } else {
                renderMessage("assistant", data.reply, data.searchResults || null);
            }
            ...
            typingEl.remove();
            renderMessage("assistant", "Tidak bisa terhubung ke server. Periksa koneksi internet kamu.", null);
        } finally {
    ...
    function renderMessage(role, content, images) {
        ...
        if (role === "assistant" && images && images.length > 0) {
            ...
            images.slice(0, 10).forEach((img) => {
                ...
                card.innerHTML = `
                    <img src="${img.medium}" alt="${escapeHtml(img.alt || 'Hasil pencarian')}" loading="lazy" class="lazy-photo-img">
                    <span class="ai-drawer-provider">${img.provider}</span>
                `;
                card.addEventListener("click", () => {
                    window.open(img.large || img.medium, "_blank", "noopener,noreferrer");
                });
    ...
    function collectHistoryForContext() {
        const bubbles = Array.from(chatMessages.querySelectorAll(".ai-message"));
